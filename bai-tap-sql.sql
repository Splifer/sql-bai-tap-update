...
where SINHVIEN.HoTenSV = 'Lê Khánh Linh'
...
select MaDT, TenDT, KinhPhi as 'Đề tài có kinh phí cao nhất'
from DETAI
where KinhPhi = (select max(KinhPhi) from DETAI)
--Câu 8: Sử dụng câu lệnh SQL xuất ra Tên khoa, Số lượng sinh viên của mỗi khoa.
select MaKhoa, count(HoTenSV) as 'Số lượng sinh viên của mỗi khoa'
from SINHVIEN
group by MaKhoa
